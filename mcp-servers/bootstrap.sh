...
if [ -n "$GHL_TOKEN_KTU" ]; then
  reg ghl-ktu "{\"type\":\"http\",\"url\":\"https://services.leadconnectorhq.com/mcp/\",\"headers\":{\"Authorization\":\"$GHL_TOKEN_KTU\",\"locationId\":\"nHLCxHPidnhV1NFzRtZZ\"}}"
else skipped+=("ghl-ktu (GHL_PIT_KTU or GHL_PIT_AGENCY)"); fi

if [ -n "$GHL_TOKEN_BTU" ]; then
  reg ghl-btu "{\"type\":\"http\",\"url\":\"https://services.leadconnectorhq.com/mcp/\",\"headers\":{\"Authorization\":\"$GHL_TOKEN_BTU\",\"locationId\":\"0uWA8M5BzHrrcJftuaDe\"}}"
