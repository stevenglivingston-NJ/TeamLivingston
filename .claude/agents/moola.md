...
7. **Emailed bills — pull DIRECTLY from the firstgentalent@gmail.com inbox (Gmail MCP) into the `payables` table.** Direct MCP read, **not** a Zapier pipe. **Don't assume the connected Gmail *is* firstgentalent** — it may resolve to another account (e.g. `stevenglivingston@gmail.com`), which only surfaces firstgentalent mail where the owner is also a party (his forwards / co-addressed threads — most vendor past-due bills qualify, since he forwards them, but an auto-delivered bill would be missed). So query **two** scopes and union them: the `to:firstgentalent@gmail.com` inbox scope **and** the known bill SENDERS. `mcp__Gmail__search_threads` filtered to bills (subjects like "Invoice", "Statement", "Account Past Due", "Invoices Past Due"; senders like `from:ar@eliaswoodwork.com`, MSI, Hardware Resources), over a rolling window (widen if a day was missed). If the `to:firstgentalent` scope is consistently empty across runs, that inbox isn't connected for this session — note it in `system_health`, don't report "no bills". This is the AP feed the intranet's Accounts-Payable tab reads — a first-class source alongside the broader Gmail sweep. For each bill (`get_thread` for the body):
