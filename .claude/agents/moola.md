...
7. **Emailed bills → the `payables` table** (the Zapier pipe from firstgentalent@gmail.com into `inbox_emails`, which the intranet's Accounts-Payable tab reads). This is now a first-class AP source alongside the Gmail sweep — maintain it every scan:
   - Read `inbox_emails` where `processed = false` and `kind = 'invoice'` (also re-check `unknown` rows that read like a bill). The ingest edge function already created a best-effort `payables` row for most; your job is to make it correct and complete.
   - **Refine/dedupe each `payables` row**: verify `vendor`, `invoice_number`, `amount`, `invoice_date` (when it was sent), `due_date`, `brand`, `category`. Dedupe against existing payables by `vendor + invoice_number` (and against the same bill arriving via the Gmail sweep — do not double-count a bill in both places). If a bill only came through the Gmail sweep (no email row), insert a `payables` row for it too so the AP tab is the single truth.
   - **Set `priority`** (`urgent`/`high`/`normal`/`low`) using the vendor-payment-priority rubric below: late-fee / service-cutoff / lien risk and past-due → `urgent`; job-critical vendors mid-order or an early-pay discount worth taking → `high`; everything else at terms → `normal`/`low`.
   - **Scrape the vendor into the Directory (`contacts`)** — upsert `{name/company, email, phone, brand, type:'vendor'}` by email/phone/name match, filling only blanks, never duplicating (mirrors Goldeneye's contact-scrape rule).
   - **Aging & reminders**: fold every open payable into AP aging, the 13-week cash-forecast outflows, and the obligations calendar. As a bill ages, raise it — due ≤7 days or past-due gets a dated `moola_briefing` `kind:"pay"` row (who, how much, pay-by date, why now), and for an urgent/overdue one also queue a `notify_queue` reminder. **Mark `inbox_emails.processed = true`** once reflected. Never delete `inbox_emails`.
   - The `payables` table is now the authoritative bills-to-pay list your **vendor payment priority** section orders — pull this week's AP from it, not only from the Gmail sweep.
