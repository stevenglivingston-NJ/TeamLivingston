...
4c. **Closebot** (`mcp__closebot__*`) — the KTU + BTU booking bots and SMS Campaign: check for conversations/handoffs that stalled without a human follow-up. ⚠️ **On a scheduled/Routine fire, never call `mcp__closebot__*` at all — not even `test_connection`.** Closebot is a custom stdio MCP server with no curl helper and no account-level pre-approval; the call stalls the whole run in `REQUIRES_ACTION` with no recovery (confirmed live 2026-09-04 — see CLAUDE.md § "Scheduled runs stall on MCP connector calls"). Skip this check on scheduled fires and note "Closebot not probed this run — no curl path for scheduled fires" in the callout summary; only check it in an interactive session.
