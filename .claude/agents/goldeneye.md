...
4d. **Perceptionist call notes — pull DIRECTLY from the firstgentalent@gmail.com inbox (Gmail MCP).** This is a direct MCP read, **not** a Zapier pipe: the connected Gmail can see that inbox, so scope your search to it — `mcp__Gmail__search_threads` with query `to:firstgentalent@gmail.com newer_than:3d` (widen the window if a day was missed), combined with your existing Perceptionist sweep. For each Perceptionist / call-note message, `get_thread` for the body, then:
