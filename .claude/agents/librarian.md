...
- Write to the **`resources`** table (NOT `intranet_records`), via the curl helper `bash mcp-servers/sb.sh '<SQL>'` (service role): `UPDATE public.resources SET drive_url = '<viewUrl>', drive_file_name = '<file or folder name>', drive_synced_at = now() WHERE id = '<matched id>'`.
...
Write to project `tguwpswcneywvscxzyef`, table `intranet_records`. **RLS is enforced — write via the curl helper `bash mcp-servers/sb.sh '<SQL>'` (service role, curl→PostgREST, not permission-gated), NOT the anon REST endpoint.**
